...
test('the stream-named family is labelled by alias and routed by SourceStream', () => {
  ...
  assert.deepEqual(grid.sources.map((s) => s.name), ['Disconnect', 'TX3_S0']);
  assert.equal(grid.destinations[0].connectedSource, '41759AEC60DF_S0');
  // By port, not by index: the Disconnect entry now occupies index 0.
  assert.equal(grid.sources.find((s) => s.port === '41759AEC60DF_S0').signalPresent, false);

  // The token that clears a destination is confirmed for this family too, so
  // the model carries it. The panel hides the column and sends this value when
  // a routed cell is clicked.
  assert.equal(grid.disconnect, '0');
  assert.equal(grid.sources[0].name, 'Disconnect');
});
